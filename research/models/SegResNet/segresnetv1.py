    ...
    def _make_down_layers(self):
        ...
        for i, item in enumerate(self.blocks_down):
            down_layer = nn.Sequential(
                self.downsample_layers[i], *[ConvNextBlock3D(dim=self.dims[i], drop_path=0.1) for _ in range(item)] 
            )
            ...
            up_layers.append(
                nn.Sequential(
                    *[
                        ...
                        *[ConvNextBlock3D(dim=sample_in_channels//2) for _ in range(blocks_up[i])]
                    ]
                )
            )
        return up_layers
