...
from building_blocks import ResBlock_d
...
class SegResNet(nn.Module):
    ...
    def _make_down_layers(self):
        ...
        for i, item in enumerate(blocks_down):
            ...
            down_layer = nn.Sequential(
                pre_conv, *[ResBlock_d(spatial_dims, layer_in_channels, norm=norm, act=self.act) for _ in range(item)]
            )
    ...
    def _make_up_layers(self):
        ...
        for i in range(n_up):
            ...
            up_layers.append(
                nn.Sequential(
                    *[
                        ResBlock_d(spatial_dims, sample_in_channels // 2, norm=norm, act=self.act)
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        # Keep the spatial dimension fixed
        x = self.convInit(x) 
